...
def generate_pdf_report(
    ...
    <b>{metrics['rendement_annualise']*100:.2f}%</b> avec une volatilité de 
    <b>{metrics['volatilite_annuelle']*100:.2f}%</b>. Le ratio de Sharpe de 
    <b>{metrics['sharpe']:.2f}</b> indique {'une bonne' if metrics['sharpe'] > 1 else 'une modérée' if metrics['sharpe'] > 0.5 else 'une faible'} 
    performance ajustée au risque.
    <br/><br/>
    La VaR à 99% de <b>{metrics['var_99_historique']*100:.2f}%</b> signifie que dans 
    99% des cas, la perte journalière ne devrait pas dépasser ce seuil. 
    L'Expected Shortfall de <b>{metrics['es_99']*100:.2f}%</b> indique la perte moyenne 
    lorsque ce seuil est dépassé.
    <br/><br/>
    Le drawdown maximal de <b>{metrics['max_drawdown']*100:.2f}%</b> représente la pire 
    baisse enregistrée sur la période. La skewness de <b>{metrics['skewness']:.3f}</b> et 
    la kurtosis de <b>{metrics['kurtosis']:.3f}</b> renseignent sur la forme de la distribution 
    des rendements (asymétrie et épaisseur des queues).
    """
